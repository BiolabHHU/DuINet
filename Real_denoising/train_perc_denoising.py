...
from utils.lpips_utils import get_features
...
layer = {"4": "relu1_2", "14": "relu3_2", "23": "relu4_2", "36": "relu5_4"}
...
model_perceptual = nn.DataParallel(vgg, device_ids=device_ids).cuda()
######### DataLoaders ###########
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):    
        ...
        GT_features = get_features(target, vgg, layers=layer)
        out_features = get_features(restored, vgg, layers=layer)

        loss1 = criterion(GT_features["relu1_2"], out_features["relu1_2"]) / (
                target.size()[1] * target.size()[2] * target.size()[3])
        loss2 = criterion(GT_features["relu3_2"], out_features["relu3_2"]) / (
                target.size()[1] * target.size()[2] * target.size()[3])
        loss3 = criterion(GT_features["relu4_2"], out_features["relu4_2"]) / (
                target.size()[1] * target.size()[2] * target.size()[3])
        loss4 = criterion(GT_features["relu5_4"], out_features["relu5_4"]) / (
                target.size()[1] * target.size()[2] * target.size()[3])
        loss5 = loss4 + loss3 + loss2 + loss1
        loss_l2 = (criterion(restored, target) / (target.size()[1] * 2))

        loss = loss_l2 + 0.05 * loss5

        loss.backward()
        optimizer.step()
        epoch_loss += loss.item()
        writer.add_scalars('training', {'training_total_loss': loss.item()
                                        }, epoch)
        writer.add_scalars('training_img', {'img_loss_l2': loss_l2.item(),
                                            'perceptual': loss5.item(),
                                            }, epoch)
